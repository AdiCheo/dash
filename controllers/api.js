...
exports.getGoogle = function(req, res, next) {
  ...
  // var redirectUrl = ["http://dash-adicheo.c9users.io:80/auth/google/callback","http://dash-adicheo.c9users.io/auth/google/callback"];
  // var oauth2Client = new auth.OAuth2(process.env.GOOGLE_ID, process.env.GOOGLE_SECRET, redirectUrl);
  ...
  async.parallel({
    ...
    authorize({
        "web": {
          "client_id": "984780978694-uoqeim00to4pq9ho25trfdbf5vsqmfj8.apps.googleusercontent.com",
          "project_id": "dash-1224",
          "auth_uri": "https://accounts.google.com/o/oauth2/auth",
          "token_uri": "https://accounts.google.com/o/oauth2/token",
          "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
          "client_secret": "8B_cLCdIItzMSWO1Fuccpeyz",
          "redirect_uris": ["http://dash-adicheo.c9users.io:80/auth/google/callback", "http://dash-adicheo.c9users.io/auth/google/callback"],
          "javascript_origins": ["http://dash-adicheo.c9users.io"]
        }
      },
      listLabels);
    ...
    */
    ...
    function authorize(credentials, callback) {
      ...
      var redirectUrl = "http://dash-adicheo.c9users.io/auth/google/callback";
      var auth = new googleAuth();
      var oauth2Client = new auth.OAuth2(clientId, clientSecret, redirectUrl);
    
      // Check if we have previously stored a token.
      
      var token = _.find(req.user.tokens, { kind: 'googleToken' });
      if (!token) {
    ...
    */
    function storeToken(token) {
        User.findById(req.user.id, function(err, user) {
          user.tokens.push({ kind: 'googleToken', googleToken: token });
    ...
    */
    function listLabels(auth) {
      ...
      gmail.users.labels.list({
        ...
        if (err) {
          console.log('The API returned an error0: ' + err);
          done(err, null);
          return;
        }
    ...
    //           }, function(err, response) {
    //             if (err) {
    //               console.log('The API returned an error1: ' + err);
    //               done(err, null);
    //               return;
    //             }
    //             var util = require('util');
